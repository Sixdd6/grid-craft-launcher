...
- R7.1 Search Modrinth and CurseForge by text, content type, Minecraft version, and loader. A hit opens a details screen with the project's description, its version list, and its image gallery (Modrinth's `gallery`, CurseForge's `screenshots`) with a full-size viewer. The description comes from the source's own markup — Modrinth's `body` markdown, CurseForge's `GET /v1/mods/{id}/description` HTML — and is converted to headings, paragraphs, bullets, code blocks, tables, rules, quotes, and images for display. Each version row also has a Notes button that opens that version's release notes (Modrinth's `changelog` field, CurseForge's `GET /v1/mods/{id}/files/{fileId}/changelog`) in a modal, rendered the same way.
...
| R7.1 | Partial | Text, type, version, and loader filters work at both sources, and a hit opens a details screen with the description, the version list, and a Gallery tab whose thumbnails open a full-size viewer with Prev/Next and Escape; Modrinth's gallery verified live, CurseForge's screenshots unverified (no key). The description renders tables, rules, quotes, nested bullets, and images, from any https host. A version's Notes button opens its release notes in a modal, rendered the same way. Modrinth verified live, description and single-version changelog included; CurseForge not verified live (no key), and both its description and changelog envelopes are unverified. |

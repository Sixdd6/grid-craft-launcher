...
fn latest_row_fields_of_no_version_reads_none() {
    ...
    let (number, id, installed, state) = latest_row_fields(&latest, None);
    assert_eq!(number, "");
    assert_eq!(id, "");
    assert_eq!(installed, "");
...
fn latest_row_fields_of_not_installed_carries_no_installed_number() {
    ...
    let (number, id, installed, state) =
        latest_row_fields(&latest, Some(&InstallState::NotInstalled));
    assert_eq!(number, "0.5.9");
    assert_eq!(id, "v2");
    assert_eq!(installed, "");
    assert_eq!(state, "not_installed");
...
fn latest_row_fields_of_installed_carries_its_own_number() {
    ...
    let (number, id, installed, state) = latest_row_fields(&latest, Some(&installed_state));
    assert_eq!(number, "0.5.9");
    assert_eq!(id, "v2");
...
fn latest_row_fields_of_older_carries_the_installed_number() {
    ...
    let (number, id, installed, state) = latest_row_fields(&latest, Some(&older_state));
    assert_eq!(number, "0.5.9");
    assert_eq!(id, "v2");
...
fn latest_row_fields_of_unknown_install_state_reads_unknown_with_no_installed_number() {
    ...
    let (number, id, installed, state) = latest_row_fields(&latest, Some(&InstallState::Unknown));
    assert_eq!(number, "0.5.9");
    assert_eq!(id, "v2");
    assert_eq!(installed, "");
...
fn a_fresh_row_is_unknown_with_no_latest_number_unlike_a_failed_lookup() {
    ...
    let (number, _, _, state) = latest_row_fields(&latest, Some(&InstallState::Unknown));
    assert_eq!(state, "unknown");
    assert_eq!(
        number, "0.5.9",
